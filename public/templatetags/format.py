...
def ansible_result(s):  # ansible_result
    ...
    for d in data:
        if d.get('status') in ["failed", "unreachable"]:
            ...
            continue
        elif d['result']['changed'] is False and d['status'] != 'ignoring':
            color = 'green'
        elif not d['result']['changed']:
            color = 'red'
        elif d['result']['changed']:
            color = 'yellow'
        msg += '''<span style="color:{color}">{host} | {task} => {status} <br>        "changed": {changed}, 
        <br>        "{task}": {data} </span><br>'''.format(
            color=color, host=d['host'], task=d['task'], status=d.get('status', 'None'),
            data=d['result'].get('msg', ''), changed=d['result'].get('changed')
        )
        if d['status'] == 'skipped':
